  ...
  return (
    <form
      onSubmit={async (e) => {
        e.preventDefault();
        if(!onAdd){
        const data = await activity.func(
          selectedColoumn?.id||0,
          formData,
        );
        if (data === "ok") {
...
>
  {activity?.inputs.map((input: any, i: number) => (
      ...
      <div className="relative group">
        {input.type === "dropDown" ? (
          ...
          <input
            ...
            className="w-full pl-4 pr-12 py-3 bg-gray-50 border border-gray-400 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all placeholder:text-gray-400"
          />
        )}
      </div>
    </div>
